...
public class SwaggerConfig {
	...
	public Docket makeDocket() {
		...
		builder = builder.apis(RequestHandlerSelectors.basePackage("com.board.controller"));

		// api()로 선택된 api중 특정 path 조건에 맞는 api를 다시 필터링하여 문서화 작업을 함
		builder = builder.paths(PathSelectors.regex("/posts/*.*"));
	...
	private ApiInfo apiInfo() {
		...
		b = b.title("Board API입니다");
		b = b.description("<h3>Board</h3>Swagger를 이용한 Board API<br>");
		b = b.version("3.0");
		ApiInfo ai = b.build();
		return ai;
	}
